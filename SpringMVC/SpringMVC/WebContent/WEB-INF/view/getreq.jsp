		...
		<table id="table" border="1" cellpadding="1">
			...
			<c:forEach items="${reqlist}" var="contracts">
				...
				<tr>
					...
					<td><c:out value="${contracts.getSub3()}" /></td> <!-- dikigoros-->
					<td><c:out value="${contracts.getSub4()}" /></td> <!-- simbolaiografos-->
					<td><c:out value="${contracts.getSub5()}" /></td> <!--imerominia -->
					<td><c:out value="${contracts.getSub6()}" /></td>  <!-- -->
					<td><c:out value="${contracts.getSub7()}" /></td> <!-- -->
					<td><c:out value="${contracts.getSub8()}" /></td> <!-- -->
					<td><c:out value="${contracts.getSub9()}" /></td> <!--dieuthinsi -->
					<td><c:out value="${contracts.getSub10()}" /></td> <!-- -->
					<td><c:out value="${contracts.getSub11()}" /></td> <!-- -->
					<td><c:out value="${contracts.getSub12()}" /></td> <!-- -->
					<td><c:out value="${contracts.getSub13()}" /></td> <!--mitronimo2 -->
					<td><c:out value="${contracts.getSub14()}" /></td> <!-- patronimo2-->
					<td><c:out value="${contracts.getSub15()}" /></td>
					<td><c:out value="${contracts.getSub16()}" /></td>
					<td><c:out value="${contracts.getSub17()}" /></td> <!--amka3 -->
					...
					<td name="Status"><c:out value="${contracts.getState()}" />
					<sec:authorize access="hasRole('LAWYER')">
						<td name="buttons">
							<button name="accept" class="accept" style="font-size: 16px">
								<span class="glyphicon glyphicon-ok"
									  onmouseover="this.style.color='blue'"
									  onmouseout="this.style.color='black'"></span>
							</button>
							<button name="decline" class="decline" style="font-size: 16px">
